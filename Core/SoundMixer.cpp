...
int16_t SoundMixer::GetOutputVolume(bool forRightChannel)
{
	...
	double tndOutput = 2.7516713261 * GetChannelOutput(AudioChannel::Triangle, forRightChannel) + 1.9493587125 * GetChannelOutput(AudioChannel::Noise, forRightChannel) + 1.0966713261 * GetChannelOutput(AudioChannel::DMC, forRightChannel);
	...
	return (int16_t)(squareVolume + tndVolume +
		...
		GetChannelOutput(AudioChannel::VRC6, forRightChannel) * 64 +
#ifndef VRC7_USE_OLD_EMU
#define VRC7_USE_NUKED
#endif

#ifdef VRC7_USE_NUKED
		GetChannelOutput(AudioChannel::VRC7, forRightChannel) * 11 +
#else
		GetChannelOutput(AudioChannel::VRC7, forRightChannel) +
#endif
		GetChannelOutput(AudioChannel::EPSM_L, forRightChannel) * 2.7 +
		GetChannelOutput(AudioChannel::EPSM_R, forRightChannel) * 2.7
	);
}
